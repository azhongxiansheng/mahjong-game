...
static func rejected(raw_code: Variant) -> ActionResolution:
	if typeof(raw_code) != TYPE_STRING_NAME:
		return null
	match raw_code:
		INVALID_ACTION, WRONG_HAND, WRONG_DECISION, WRONG_SEAT, WRONG_PHASE, NOT_OFFERED, ALREADY_RESPONDED, ENTITY_NOT_FOUND, RULE_REJECTED, NOT_ENABLED, REPLAY_MISMATCH:
			pass
		_:
			return null
	var result := ActionResolution.new()
