...
func test_card_system() -> void:
	...
	print("%s 和 %s 能配对: %s" % [card_a.get_name(), card_b.get_name(), card_a.can_pair_with(card_b)])
	print("%s 和 %s 能配对: %s" % [card_a.get_name(), card_c.get_name(), card_a.can_pair_with(card_c)])
	await get_tree().create_timer(0.5).timeout
	...
	for num in range(1, 10):
		...
		print("%s 的等级: %s" % [test_card.get_name(), test_card.get_rank()])
