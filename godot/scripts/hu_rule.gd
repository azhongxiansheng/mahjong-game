...
# 计算胡牌的番数
static func calculate_fan(win_result: WinChecker.WinResult, hand: CardHand) -> int:
	"""
	根据胡牌结果计算番数
	返回：总番数
	"""
...
# 检查是否全是刻子
static func is_all_pung(win_result: WinChecker.WinResult) -> bool:
	"""检查是否全是刻子（包括杠）"""
...
# 检查是否全是顺子
static func is_all_chow(win_result: WinChecker.WinResult) -> bool:
	"""检查是否全是顺子"""
	if not win_result or win_result.win_patterns.is_empty():
		return false
...
# 获取胡牌类型名称
static func get_hu_type_name(win_result: WinChecker.WinResult, hand: CardHand) -> String:
	"""获取胡牌类型的名称"""
...
# 打印胡牌信息
static func print_win_info(win_result: WinChecker.WinResult, hand: CardHand) -> void:
	"""打印详细的胡牌信息"""
	if not win_result or not win_result.can_win:
		print("✗ 未能胡牌")
		return
