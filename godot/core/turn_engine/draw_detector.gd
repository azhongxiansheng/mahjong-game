...
# 包装 0d 流局判定，从 TableState 提取所需切面。
# 九种九牌 / 三家和了 不放这（属玩家选择/时机性，turn_engine 直接调 0d 函数）。

# 牌墙耗尽流局：live wall 摸尽
static func should_exhaustive_draw(state: TableState) -> bool:
	return state.wall.live_wall_size() == 0

# 四风连打：4 家都弃了至少 1 张 + 仍在第 1 巡 + 4 家第 1 弃同字风
static func is_suufon_renda(state: TableState) -> bool:
...
static func is_suucha_riichi(state: TableState) -> bool:
	var arr: Array = []
	for seat in state.seats:
...
static func is_suukantsu_sanra(state: TableState) -> bool:
	var kan_seats: Array = []
	for seat in state.seats:
