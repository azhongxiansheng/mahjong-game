...
static func simulate(config: Dictionary) -> Dictionary:
	...
	# M10 Path A：是否启用 HeuristicAi shanten-aware 弃牌（自动隐式开 heuristic_ai）
	var use_shanten_ai: bool = bool(config.get("shanten_ai", false))
	...
	for i in range(runs):
		var run_outcome := _simulate_one(base_seed + i, starter, max_nodes, pick_strategy, use_heuristic_ai, starting_hp_override, rank_hp_delta_override, fair_tiebreak, ai_seat_abilities, use_shanten_ai)
		if run_outcome.won:
			completed += 1
...
static func _simulate_one(run_seed: int, starter: StringName, max_nodes: int, pick_strategy: String, use_heuristic_ai: bool = false, starting_hp_override: int = 0, rank_hp_delta_override: Array = [], fair_tiebreak: bool = false, ai_seat_abilities: bool = false, use_shanten_ai: bool = false) -> Dictionary:
	...
	while not rs.finished and nodes_visited < max_nodes:
		...
		if NodeKind.is_battle(current.kind):
			...
			var node_stats: Dictionary = BattleNodeRunner.run_battle_with_stats(node_seed, boss_id, ability_ids, use_heuristic_ai, tile_variants, tb_seed, ai_seed, current.session_kind, use_shanten_ai)
