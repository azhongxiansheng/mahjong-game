...
func test_drain_transfers_20_percent_on_opponent_win():
	# balance tune-1：soul_drain_fraction 0.30 → 0.20
	var arr := _setup()
	var reg: SkillRegistry = arr[0]
	var st: BattleState = arr[1]
	var sched: SkillScheduler = arr[2]
	var sk := _make_skill()
	# holder_seat=0 持牌；座位 1 胡 8000 → 0 拿 20% × 8000 = 1600
	var ti := TileInstance.make(Tile.new(TileId.HATSU), 2, sk)
	ti.holder_seat = 0
	reg.register(sk, ti)
	sched.emit_event(BattleEvent.make(&"WIN_DECLARED", 1, null, {"points_won": 8000}))
	assert_eq(st.scores[0], 25000 + 1600)
	assert_eq(st.scores[1], 25000 - 1600)
