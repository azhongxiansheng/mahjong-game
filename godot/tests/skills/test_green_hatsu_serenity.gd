...
func test_serenity_adds_2_han_when_holder_is_winner():
	# tune-2：green_hatsu_serenity_han_bonus 1 → 2
	var ctx := _setup()
	...
	assert_eq(int(out_ctx.han_deltas.get(1, 0)), 2, "holder=1 自胡 → +2 番")
