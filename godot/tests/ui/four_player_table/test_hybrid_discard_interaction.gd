...
func test_immediate_pick_ignores_followup_flick_to_avoid_duplicate_action() -> void:
	var table := MahjongTable3D.new()
	add_child_autofree(table)
	await get_tree().process_frame
	var tile := _entity(TileId.S6, 811)
	table._hand_tiles = [tile]
	table.set_hand_clickable(true)
	table.set_hand_activation_mode(&"immediate")
	watch_signals(table)
	...
	_click(table, 811)
	table.call("_on_tile_flicked", 811)
	assert_signal_emit_count(table, "player_card_clicked", 1,
		"吃碰/杠候选的立即模式已在按下时提交，上推不得重复提交")
