...
# 立直棒已归入左上宝牌区；中央盘即使收到兼容 setter 也不再重复绘制。
func test_center_never_draws_riichi_sticks_or_glow() -> void:
	...
	assert_eq(panel.count_riichi_stick_visuals(), 0, "中央盘不得重复画立直棒")
	assert_false(panel.is_riichi_stick_glow_active(), "中央盘不得保留立直棒呼吸光")
	panel.set_riichi_sticks(0)
	assert_eq(panel.count_riichi_stick_visuals(), 0)
