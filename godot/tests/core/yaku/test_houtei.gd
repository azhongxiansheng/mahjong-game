...
func _make_wc(is_houtei: bool, is_tsumo: bool) -> WinContext:
	var hand := Hand.new()
	for tid in [
...
func test_houtei_with_ron():
	var e := Houtei.detect(_make_wc(true, false))
	assert_not_null(e)

func test_houtei_with_tsumo_returns_null():
	assert_null(Houtei.detect(_make_wc(true, true)), "河底捞鱼必须是荣胡")
