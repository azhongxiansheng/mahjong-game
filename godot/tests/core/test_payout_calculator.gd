...
# PayoutCalculator: 把基本点 + ScoreContext 转成 {payer_seat: points} 字典。
...
func test_dealer_ron_base_480():
	...
	var ctx := ScoreContext.ron(Tile.new(TileId.W5), TileId.E, TileId.E, 0, 0, 1)
	var p := PayoutCalculator.payout(480, ctx)
	assert_eq(p[1], 2900)

func test_dealer_ron_mangan():
	# 庄荣胡满贯 base=2000：base*6 = 12000
	var ctx := ScoreContext.ron(Tile.new(TileId.W5), TileId.E, TileId.E, 0, 0, 2)
...
func test_pao_non_dealer_tsumo_full_burden():
	...
	var ctx := ScoreContext.tsumo(Tile.new(TileId.HAKU), TileId.E, TileId.S_WIND, 0, 1)
	ctx.pao_seat = 3
	var p := PayoutCalculator.payout(2000, ctx)
	assert_eq(p[3], 8000)
