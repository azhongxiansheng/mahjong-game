...
# ScoreContext: 胡牌瞬时上下文（区别于持续的 BattleState）
...
func test_default_values():
	...
	assert_eq(ctx.loser_seat, ScoreContext.NO_SEAT)
	assert_eq(ctx.pao_seat, ScoreContext.NO_SEAT)

func test_tsumo_factory():
	var ctx := ScoreContext.tsumo(
		Tile.new(TileId.W5),
		TileId.E,        # round_wind
	...
	assert_eq(ctx.loser_seat, ScoreContext.NO_SEAT, "自摸无放铳者")

func test_ron_factory():
	var ctx := ScoreContext.ron(
		Tile.new(TileId.T3),
		TileId.E,
		TileId.E,        # 庄家自风也是东
		0,
...
func test_winner_is_dealer_helper():
	var ctx_dealer := ScoreContext.tsumo(Tile.new(TileId.W1), TileId.E, TileId.E, 0, 0)
	assert_true(ctx_dealer.winner_is_dealer())
	var ctx_non := ScoreContext.tsumo(Tile.new(TileId.W1), TileId.E, TileId.S_WIND, 0, 1)
...
func test_honba_and_sticks():
	var ctx := ScoreContext.ron(Tile.new(TileId.T3), TileId.E, TileId.E, 0, 0, 2)
	ctx.honba = 3
	ctx.riichi_sticks = 2
...
func test_pao_seat_settable():
	var ctx := ScoreContext.ron(Tile.new(TileId.HAKU), TileId.E, TileId.S_WIND, 0, 1, 2)
	ctx.pao_seat = 3
	assert_eq(ctx.pao_seat, 3)
	assert_true(ctx.has_pao())

func test_no_seat_constant_is_negative():
	assert_lt(ScoreContext.NO_SEAT, 0, "用负值哨兵区分有效 seat (0..3)")
