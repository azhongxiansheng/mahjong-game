...
func test_ai_skips_claim_when_riichi():
	var ai := HeuristicAi.new(42)
	var seat := _seat_with_hand([
		TileId.W5, TileId.W5,
		TileId.T1, TileId.T2, TileId.T3,
	...
	var result: Dictionary = ai.decide_claim(seat, TileId.W5, 0)
	assert_eq(result.get("action", ""), "skip", "riichi blocks claims")
...
func test_ai_skips_self_discard():
	...
	var result: Dictionary = ai.decide_claim(seat, TileId.W5, 0)
	assert_eq(result.get("action", ""), "skip", "cannot claim own discard")
...
func test_ai_skips_ankan_when_riichi():
	...
	seat.riichi.declared = true
	var result: Dictionary = ai.decide_self_kan(seat)
	assert_eq(result.get("action", ""), "skip", "riichi blocks self-kan")
