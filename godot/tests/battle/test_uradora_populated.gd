...
# 裏 dora 必须在每局开始时预置进 dora_indicators.uradora_tiles() —
...
func test_initial_state_has_at_least_one_hidden_uradora() -> void:
	...
	assert_gt(bc.state.dora_indicators.uradora_tiles().size(), 0,
...
func test_visible_and_hidden_count_match_after_kan() -> void:
	...
	var v0: int = bc.state.dora_indicators.visible_tiles().size()
	var u0: int = bc.state.dora_indicators.uradora_tiles().size()
	assert_eq(v0, 1, "起手 1 张明 dora")
	assert_eq(u0, 1, "起手 1 张裏 dora")
	# 直接调内部方法翻新 dora
	bc.engine._reveal_new_dora()
	assert_eq(bc.state.dora_indicators.visible_tiles().size(), v0 + 1)
	assert_eq(bc.state.dora_indicators.uradora_tiles().size(), u0 + 1)
