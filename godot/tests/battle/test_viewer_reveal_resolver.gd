...
func _record(st: BattleState, holder_seat: int, viewer_seat: int,
	...
	var tile: Tile = st.seats[holder_seat].hand.tile_at(hand_index)
	var instance := TileSkillAnchor.make(tile, holder_seat)
...
func test_groups_only_authorized_live_tiles_by_holder_and_deduplicates() -> void:
	...
	var revealed := (grouped[1] as Array)[0] as TileSkillAnchor
	assert_eq(revealed.tile.instance_id,
		(authorized.tile as TileSkillAnchor).tile.instance_id)
