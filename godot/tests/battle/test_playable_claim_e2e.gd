...
func test_player_claim_probe_fires_during_hand() -> void:
	# 不 bind UI → _action_panel == null → _try_player_claim_async 第一行 return
	...
	# 当前实现下 PROBE 在 _try_player_claim_async 内部，需要 _action_panel != null 才会跑到那行
...
# 关键 verify：async 路径 + 不 bind UI 也不会 emit PROBE 因为 _action_panel == null 早 return。
