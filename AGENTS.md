...
- 全量 GUT、import、构建等高输出命令优先只回传最终 totals、失败/错误、退出码与本次新增 warning；仓库既有 warning 不重复展开完整堆栈。
- 完整累计 diff 必须至少独立审查一次；返工后优先复读完整变更清单、受影响文件和关键片段，不重复输出未变化的大段 diff。
- 需要保留原始长日志时写到仓库外 `/tmp`，对话中用 `rg` / `tail` 提取与当前结论直接相关的证据；不得因此隐藏失败、跳过门禁或把“输出被截断”误报为通过。
