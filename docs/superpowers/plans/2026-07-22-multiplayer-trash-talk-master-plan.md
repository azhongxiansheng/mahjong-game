...
1. **#225 入口切换**：先写主场景/导航 Red，再建立大厅最小场景并从大厅启动现有可玩桌面。
2. **#226 肉鸽依赖退出生产**：按 E0 清单逐项解除 RunState、SaveSystem、经济/章节 UI 与新入口耦合；保留历史文件，直到没有生产引用后再按 Issue 范围处理。
3. **#227 大厅场景**：按 ASCII 搭 1600×900 布局和响应式锚点；先几何测试，后视觉实现。
4. **#228 规则抽屉**：建立选择状态和 `GameSessionConfig` 输出；覆盖键鼠焦点、取消和返回。
5. **#229 图鉴/设置**：复用既有 SettingsManager/AudioManager 的 BGM/SFX 能力；不加入语音设置。
6. **#230 角色原创化**：先建立 12 项能力映射测试，再替换 ID、文案、立绘和引用；PNG 更新后执行 Godot import。
...
- **#231**：新增纯数据 `GameSessionConfig`，稳定枚举值与验证/序列化；大厅只传该对象，不传 RunState。
