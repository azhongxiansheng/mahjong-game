...
if [[ -e "$wrapper_path" ]]; then
  print -u2 "visible launcher already exists: $wrapper_path"
  exit 73
fi

printf '#!/bin/zsh\ncd %q\nif %q list-sessions >/dev/null 2>&1; then\n  %q set-option -g mouse on\n  %q set-option -g history-limit 50000\nfi\nexec %q -f %q new-session -s %q -c %q %q\n' \
  "$task_worktree" "$tmux_bin" "$tmux_bin" "$tmux_bin" "$tmux_bin" \
  "$tmux_config" "$session_name" "$task_worktree" "$runner_path" > "$wrapper_path"
